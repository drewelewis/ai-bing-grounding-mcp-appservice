...
def main():
    ...
    print("🤖 Creating Bing Grounding Agents...")
    print()
    ...
    default_instructions = defaults_config.get("instructions", 
        "You are a web search assistant. Use Bing Search to find current information before answering. Include citations for all facts.")
    
    # Get individual agent configurations
    agent_configs = agents_config.get("agents", [])
    ...
    if not enabled_agents:
        print("⚠️ No agents in config, using defaults")
        enabled_agents = [
            {"name": "gpt4o_1", "model": "gpt-4o", "weight": 100, "enabled": True},
            {"name": "gpt41mini_1", "model": "gpt-4.1-mini", "weight": 100, "enabled": True},
        ]
    ...
    for agent_cfg in enabled_agents:
        print(f"   {agent_cfg['name']} ({agent_cfg['model']}): {agent_cfg.get('weight', 100)}% weight")
    ...
    try:
        ...
        try:
            ...
            for agent in existing_agents:
                if agent.name and agent.name.startswith("agent_bing_"):
                    ...
                    deleted_count += 1
            if deleted_count > 0:
                print(f"🗑️ Deleted {deleted_count} old agent(s)")
            else:
        ...
        # Create each individual agent from config
        for agent_cfg in enabled_agents:
            agent_name = f"agent_bing_{agent_cfg['name']}"
            model_name = agent_cfg["model"]
            weight = agent_cfg.get("weight", 100)
            instructions = agent_cfg.get("instructions", default_instructions)
            
            print(f"  [{agent_counter}] Creating {agent_name} ({model_name}, weight: {weight}%)...", end=" ")
            ...
            try:
                ...
                env_key = f"AZURE_AI_AGENT_{agent_cfg['name'].upper()}"
        ...
        print()
        print(f"\n✅ Successfully created {len(created_agents)}/{total_agents} agents")
        print()
        
        if created_agents:
            # Set the first agent as the default
            set_env_value("AZURE_AI_AGENT_ID", created_agents[0]["id"])
            print(f"🎯 Default agent: {created_agents[0]['name']} ({created_agents[0]['id']})")
            print()
            ...
            for model, agents in models.items():
                print(f"\n  {model}:")
                for agent in agents:
                    agent_key = agent['name'].replace('agent_bing_', '').upper()
                    print(f"    AZURE_AI_AGENT_{agent_key}={agent['id']} (weight: {agent['weight']}%)")
            
            print()
            print("✅ Agents are ready to use!")
        else:
            print("⚠️  No agents were created successfully")
            sys.exit(1)
